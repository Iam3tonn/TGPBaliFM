...
def start(update, context):
    update.message.reply_text('Отправьте ссылку, чтобы я смогу сделать генерацию статьи по ней.')

def handle_message(update, context):
    user_link = update.message.text
    with open('combined_data.json', 'r', encoding='utf-8') as file:  # Указываем кодировку UTF-8
        data = json.load(file)
        found = False
        for item in data:
            if item['link'] == user_link:
                found = True
                context.user_data['last_message'] = item
...
def button(update, context):
    query = update.callback_query
    query.answer()
    item = context.user_data.get('last_message')
    if item:
        prompt = generate_prompt(item, query.data)
        send_to_chatgpt(update, context, prompt)

def main():
    # Создание Updater и получение Dispatcher для регистрации обработчиков
    ...
    dp.add_handler(CallbackQueryHandler(handle_user_choice))
